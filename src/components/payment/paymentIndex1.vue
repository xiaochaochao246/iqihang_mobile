      ...
      <div class="padding-top-70">
        ...
        <div class="pay-paument-list">
          ...
          <div class="pay-paument-list-div pay-paument-list-pay">
            ...
            <ul>
              <li :class="{ 'active' : isA, '': !isA}" @click='toggle' v-show="isShow"><img src="./paymentImg/pay-zfb.png"/>支付宝<span></span></li>
              <li :class="{ 'active' : isB, '': !isB}" @click='toggleB'><img src="./paymentImg/pay-wx.png"/>微信<span></span></li>
            </ul>
            <div class="pay-paument-list-pad"></div>
          </div>
        ...
        <div class="pay-paument-botton">
          ...
          <div class="shop-bottom">
            ...
            <button @click="Orderflag && ordersub()">提交订单</button>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    ordercontrol() {
      //控制购物劵页面显示
      this.orderstat = "2";
      //        this.axios.post('/api/promotion/login1/pageView/calculation', {
      ...
      //          .then(response => {
      //            if(response.data.status == 0){
      //              this.discountPrice =parseFloat(response.data.data.totalPrice).toFixed(2);//于优惠券做对比的价格
      //            }
      //          }).catch(err => {
      //          console.log(err)
      //        })
