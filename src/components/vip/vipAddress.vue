<template>
  <div class="vip-index">
    <div v-show="addState">
      <header-right :htitle="htitleText" :rightTitle="rightwords" @childReturn="allControl" @adddata="addEvent"></header-right>
        ...
        <div class="vip-news-list" v-for="(item,index) in vipUserUrl" :key="index">
          ...
          <div class="vip-news-list-foter">
            <div class="vip-news-list-foter-lf" >
              <span :class="item.addressCommon == 1 ? 'active': '' " @click="defaultAddress(index)"></span>
              默认地址
            </div>
            <div class="vip-news-list-foter-rt">
              <router-link :to="{ path: 'addmod', query:{url:'address',common:item.addressCommon,name:item.vipName,address:item.address,tel:item.phone,delivery:item.delivery,id:item.id,userid:userId}}"> <button>编辑</button></router-link>
...
export default {
  ...
  methods:{
    ...
    vipUserId(){//获取用户地址
      this.axios.get('/api/party/user/v2/user/session', {
        ...
        .then(response => {
          ...
          this.axios.post('/api/party/sysAddress/v1/employee/queryAll', {
            userId:this.userId
          })
            .then(response => {
    ...
    defaultAddress(num) {//设为默认地址
      this.judgeJogin()
      this.axios.post('/api/party/sysAddress/v1/employee/up', {
        addressId:this.vipUserUrl[num].id,
        addressName:this.vipUserUrl[num].vipName,
        address:this.vipUserUrl[num].address,
        delivery:this.vipUserUrl[num].delivery,
        phone:this.vipUserUrl[num].phone,
        common:'1',
        userId:this.userId
      })
        .then(response => {
    ...
    submitForm() {//地址提交
      ...
      if(this.ruleForm.name == ""){
        ...
        this.ruleForm.region = this.myAddressProvince +' ' + this. myAddressCity +' ' +  this.myAddresscounty;
