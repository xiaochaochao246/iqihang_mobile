    ...
    <div class="loginbg">
      ...
      <div class="login-three">
        ...
        <div class="login-three-bm">
          <span v-show="!wxShow" id="qqLoginBtn" @click="qqLogin()"><img  src="./loginImg/vip-qq.png"/></span>
          <span v-show="wxShow" @click="wxLogin()"><img src="./loginImg/vip-wx.png"/></span>
          <!--<span id="wb_connect_btn"><img src="./loginImg/vip-wb.png"/></span>-->
  ...
  methods:{
    ...
    qqLogin(){
      ...
      if(this.$route.query.id !== null && this.$route.query.id && this.$route.query.id != 'undefined'){
        ...
        if(this.$route.query.zhuan){//判断是否是从专题过来
          QC.Login.showPopup({
            appId:"101375266",
            redirectURI:encodeURIComponent("http://mobile.iqihang.com/bbb?id="+this.$route.query.id+"&zhuan=1")
          });
        }else{
          QC.Login.showPopup({
            appId:"101375266",
            redirectURI:encodeURIComponent("http://mobile.iqihang.com/bbb?id="+this.$route.query.id)
          });
        }
      }else if(this.$route.query.id  == null){
        QC.Login.showPopup({
          appId:"101375266",
          redirectURI:encodeURIComponent("http://mobile.iqihang.com/bbb")
        });
    ...
    wxLogin(){
      if(this.$route.query.zhuan){
        window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8128e75740e730a2&redirect_uri=http://mobile.iqihang.com/bbb%3Fid=${this.$route.query.id}%26zhuan=1&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect`;
      }else{
        window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8128e75740e730a2&redirect_uri=http://mobile.iqihang.com/bbb%3Fid=${this.$route.query.id}&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect`;
      }
    },
//    wbLogin(){//微博三方登录
...
//    },
    ...
    getopenid(){
      ...
      if(ua.match(/MicroMessenger/i) == 'micromessenger'){
        ...
        if ("undefined" != typeof  this.$route.query.code){
          ...
          if(this.$route.query.zhuan){//判断是否是从专题过来
            //设置session
            window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8128e75740e730a2&redirect_uri=http://mobile.iqihang.com/qqq%3Fid=${this.$route.query.id}%26zhuan=1&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect`;
          }else{
            window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8128e75740e730a2&redirect_uri=http://mobile.iqihang.com/qqq%3Fid=${this.$route.query.id}&response_type=code&scope=snsapi_base&state=STATE#wechat_redirect`;
          }
        }
      }
    },
    register(){
      if(this.$route.query.zt){
        this.$router.push({path: '/register',query:{id:this.$route.query.zt,zhuan:1}})
      }else  if(this.$route.query.id){
        this.$router.push({path: '/register',query:{id:this.$route.query.id}})
      }else{
...
.login-three-bm span:nth-child(2){
  float: right;
  background: url("./loginImg/vip-wb.png");
  background-size: 0.8rem 0.8rem;
}
.loginbg-ct-input{
  position: relative;
