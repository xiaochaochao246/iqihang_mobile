    ...
    methods:{
      ...
      qqLogin(){
        ...
        if(this.$route.query.id !== null && this.$route.query.id && this.$route.query.id != 'undefined'){
          ...
          if(this.$route.query.zhuan){//判断是否是从专题过来
            QC.Login.showPopup({
              appId:"101375266",
              redirectURI:encodeURIComponent("http://mobile.iqihang.com/binding?id="+this.$route.query.id+"&zhuan=1")
            });
          }else{
            QC.Login.showPopup({
              appId:"101375266",
              redirectURI:encodeURIComponent("http://mobile.iqihang.com/binding?id="+this.$route.query.id)
            });
          }
        }else if(this.$route.query.id  == null){
          QC.Login.showPopup({
            appId:"101375266",
            redirectURI:encodeURIComponent("http://mobile.iqihang.com/binding")
          });
      ...
      wxLogin(){
        if(this.$route.query.zhuan){
          window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8128e75740e730a2&redirect_uri=http://mobile.iqihang.com/binding%3Fid=${this.$route.query.id}%26zhuan=1&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect`;
        }else{
          window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8128e75740e730a2&redirect_uri=http://mobile.iqihang.com/binding%3Fid=${this.$route.query.id}&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect`;
        }
      },
//    wbLogin(){//微博三方登录
...
//    },
              ...
              if(response.data.status == "0"){
                ...
                setTimeout(() => {
                  if(this.$route.query.id !== null && this.$route.query.id && this.$route.query.id != 'undefined'){
                    ...
                    }else if((this.$route.query.id).indexOf(',') != -1 || /^\d+/.test(this.$route.query.id)){
                      this.$router.push({ path: '/payment', query:{paycid:this.$route.query.id}})
                    }else{
        ...
        if(ua.match(/MicroMessenger/i) == 'micromessenger'){
          ...
          if ("undefined" != typeof this.$route.query.code){
            ...
            if(this.$route.query.zhuan){//判断是否是从专题过来
              ...
              window.location.href = `https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx8128e75740e730a2&redirect_uri=http://mobile.iqihang.com/login%3Fid=${this.$route.query.id}&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect`;
            }
          }
        };
      },
      register(){
        if(this.$route.query.zhuan){
          this.$router.push({path: '/register',query:{id:this.$route.query.id,zhuan:1}})
        }else if(this.$route.query.id && this.$route.query.zhuan == null){
